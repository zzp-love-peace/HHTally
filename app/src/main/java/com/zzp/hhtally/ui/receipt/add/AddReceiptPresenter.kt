...
import com.zzp.hhtally.network.textin.*
...
class AddReceiptPresenter(baseView: IAddReceiptView) : BasePresenter<IAddReceiptView>(baseView) {
    ...
    fun analyzeImage(image: Bitmap?) {
        ...
        thread {
            ...
            val typeDewarpData = object : TypeToken<TextInResult<DewarpData>>() {}.type

            val resultDewarp: TextInResult<DewarpData> =
                gson.fromJson(TextInUtil.imgDewarp(bitmap2ByteArray(image)), typeDewarpData)

            if (resultDewarp.code == 200) {
                val typeCropEnhanceData =
                    object : TypeToken<TextInResult<CropEnhanceData>>() {}.type

                val resultCropEnhance: TextInResult<CropEnhanceData> = gson.fromJson(
                    TextInUtil.imgCropEnhance(
                        base642ByteArray(resultDewarp.result.image)
                    ), typeCropEnhanceData
                )

                if (resultCropEnhance.code == 200) {
                    val typeBillsCropData =
                        object : TypeToken<TextInResult<BillsCropData>>() {}.type

                    val result: TextInResult<BillsCropData> =
                        gson.fromJson(
                            TextInUtil.billsCrop(base642ByteArray(resultCropEnhance.result.image_list[0].image)),
                            typeBillsCropData
                        )
                                ...
                                result.result.object_list[0].apply {
                                    ...
                                    when (this.type) {
                                        ...
                                        "vat_special_invoice", "vat_electronic_special_invoice", "vat_electronic_invoice", "vat_common_invoice", "vat_electronic_toll_invoice", "machine_printed_invoice", "blockchain_electronic_invoice", "vat_electronic_invoice_new", "vat_electronic_special_invoice_new" -> {
                                            for (item in this.item_list) {
                                                when (item.key) {
                                                    "vat_invoice_price_list" -> billPrice =
                                                        item.value
                                                    "vat_invoice_issue_date" -> billTime =
                                                        item.value
                                                    "vat_invoice_seller_name" -> billShopkeeper =
                                                        item.value
                                                }
                                            }
                                        }
                    ...
                    activity.runOnUiThread {
                        resultCropEnhance.message.showToast()
                    }
                }

            } else {
                activity.runOnUiThread {
                    resultDewarp.message.showToast()
                }
            }
        }
    }
